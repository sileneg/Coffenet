...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $query = "UPDATE usuarios SET firstname = ?, lastname = ?, address = ?, phone = ?, email = ?, username = ?, user_level = ? WHERE id = ?";
    $stmt = $pdo->prepare($query);
    if ($stmt->execute([$firstname, $lastname, $address, $phone, $email, $username, $user_level, $id])) {
        echo "<script>alert('Usuario actualizado correctamente.'); window.location.href = 'consultar_usuarios.php';</script>";
    } else {
        echo "<script>alert('Error al actualizar el usuario.'); window.location.href = 'consultar_usuarios.php';</script>";
    }
    exit();
}
            ...
            <form action="editar_usuario.php" method="post">
                ...
                <div class="form-group">
                    ...
                    <select id="user_level" name="user_level" class="form-control" required>
                        <option value="1" <?php if ($user['user_level'] == 1) echo 'selected'; ?>>Administrador</option>
                        <option value="2" <?php if ($user['user_level'] == 2) echo 'selected'; ?>>Propietario</option>
                        <option value="3" <?php if ($user['user_level'] == 3) echo 'selected'; ?>>Vendedor</option>
                        <option value="4" <?php if ($user['user_level'] == 4) echo 'selected'; ?>>Cliente</option>
                    </select>
                </div>
